...
<head>
  ...
  <title>Quốc Trí Travel - Dịch vụ du lịch chuyên nghiệp</title>
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    body{font-family:Arial, Helvetica, sans-serif;line-height:1.6;background:#f5f5f5;color:#333;}
    header{
    ...
    header h1{
      position:relative;font-size:3em;padding:20px;border-radius:10px;
      background:rgba(0,0,0,0.3);
    }
    nav{background:#008080;padding:10px;text-align:center;position:sticky;top:0;z-index:10;}
    nav a{color:white;text-decoration:none;margin:0 15px;font-weight:bold;transition:0.3s;}
    nav a:hover{color:#ffda44;text-shadow:0 0 5px #000;}
    section{padding:50px 20px;max-width:1200px;margin:auto;}
    h2{color:#008080;margin-bottom:30px;text-align:center;font-size:2em;}
    .grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:25px;}
    .card{
      background:white;border-radius:12px;overflow:hidden;
      box-shadow:0 0 12px rgba(0,0,0,0.15);transition:transform 0.3s;
    }
    .card:hover{transform:translateY(-6px);}
    .card img{width:100%;height:180px;object-fit:cover;}
    .card-content{padding:20px;text-align:center;}
    .card h3{margin-bottom:10px;color:#006666;}
    .card p{font-size:0.95em;}
    .btn{
      display:inline-block;background:#ffda44;color:#333;
      padding:10px 18px;margin-top:12px;border-radius:6px;
      font-weight:bold;text-decoration:none;transition:0.3s;
    }
    .btn:hover{background:#ffc107;}
    .contact{
      background:#008080;color:white;padding:40px;border-radius:12px;
      text-align:center;max-width:800px;margin:auto;
    }
    .contact input, .contact textarea{
      width:90%;padding:10px;margin:10px auto;border:none;border-radius:6px;
      font-size:1em;
    }
    .contact button{
      background:#ffda44;border:none;padding:12px 25px;border-radius:6px;
      font-weight:bold;cursor:pointer;transition:0.3s;
    }
    .contact button:hover{background:#ffc107;}
